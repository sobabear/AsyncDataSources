...
open class TableViewItemsController<CollectionType>: NSObject, UITableViewDataSource
where CollectionType: RandomAccessCollection,
      CollectionType: Equatable,
  ...
  public var rowAnimations = (
    ...
    delete: UITableView.RowAnimation.fade
  )
  ...
  func updateCollection(_ items: CollectionType) {
    ...
    // Calculate the changes between the old collection and the new one.
    _ = delta(newList: items, oldList: collection)
    // Perform batch updates to reflect the changes in the table view.
    tableView.beginUpdates()
    for sectionIndex in 0..<items.count {
      let rowAtIndex = fromRow(sectionIndex)
  ...
  public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    //        guard indexPath.section < collection.count, indexPath.row < collection[indexPath.section].count else { return .init() }
    return cellFactory(self, tableView, indexPath, collection[indexPath.section][indexPath.row])
  }
